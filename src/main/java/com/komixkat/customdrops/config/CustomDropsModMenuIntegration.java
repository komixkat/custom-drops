...
public final class CustomDropsModMenuIntegration implements ModMenuApi {
    ...
    public ConfigScreenFactory<?> getModConfigScreenFactory() {
        return parent -> {
            ...
            ConfigBuilder builder = ConfigBuilder.create()
                ...
                .setTitle(Component.translatable("customdrops.config.title"))
                .setSavingRunnable(() -> {
                    finalizers.forEach(Runnable::run);
                    ConfigLoader.save(
                        net.fabricmc.loader.api.FabricLoader.getInstance().getConfigDir().resolve(CustomDropsMod.MOD_ID),
                        config);
                });

            ConfigEntryBuilder eb = builder.entryBuilder();

            buildMobDropsCategory(builder, eb, config, finalizers);
            buildBlockDropsCategory(builder, eb, config, finalizers);
            buildChestLootCategory(builder, eb, config, finalizers);
            buildFishingLootCategory(builder, eb, config, finalizers);
            buildEquipmentCategory(builder, eb, config, finalizers);
            buildBrowseCategory(builder, eb);
            buildServerInfoCategory(builder, eb);
            buildPresetsCategory(builder, eb, config);

            return builder.build();
        };
    }

    private void buildMobDropsCategory(ConfigBuilder builder, ConfigEntryBuilder eb, CustomDropsConfig config, List<Runnable> finalizers) {
        ...
        for (MobDropEntry e : config.mobDrops()) {
            existing.add(new EntrySeed(e.targetId(), e.pool(), e.conditions(), e.replaceVanillaTable()));
        }
    ...
    private void buildBlockDropsCategory(ConfigBuilder builder, ConfigEntryBuilder eb, CustomDropsConfig config, List<Runnable> finalizers) {
        ...
        List<EntrySeed> existing = new ArrayList<>();
        for (BlockDropEntry e : config.blockDrops()) {
            existing.add(new EntrySeed(e.targetId(), e.pool(), e.conditions(), e.replaceVanillaTable()));
        }
    ...
    private void buildChestLootCategory(ConfigBuilder builder, ConfigEntryBuilder eb, CustomDropsConfig config, List<Runnable> finalizers) {
        ...
        List<EntrySeed> existing = new ArrayList<>();
        for (ChestLootEntry e : config.chestLoot()) {
            existing.add(new EntrySeed(e.targetLootTableId(), e.pool(), e.conditions(), e.replaceVanillaTable()));
        }
    ...
    private void buildFishingLootCategory(ConfigBuilder builder, ConfigEntryBuilder eb, CustomDropsConfig config, List<Runnable> finalizers) {
        ...
        List<EntrySeed> existing = new ArrayList<>();
        for (FishingLootEntry e : config.fishingLoot()) {
            existing.add(new EntrySeed(e.targetLootTableId(), e.pool(), e.conditions(), e.replaceVanillaTable()));
        }
    ...
    private void buildSlots(ConfigCategory category, ConfigEntryBuilder eb, List<EntrySeed> existing,
    ...
    private void buildEquipmentCategory(ConfigBuilder builder, ConfigEntryBuilder eb, CustomDropsConfig config, List<Runnable> finalizers) {
    ...
    private void buildBrowseCategory(ConfigBuilder builder, ConfigEntryBuilder eb) {
        ConfigCategory browse = builder.getOrCreateCategory(Component.translatable("customdrops.config.category.browse"));
    ...
    private void buildServerInfoCategory(ConfigBuilder builder, ConfigEntryBuilder eb) {
        ConfigCategory serverInfo = builder.getOrCreateCategory(Component.translatable("customdrops.config.category.serverInfo"));
    ...
    private void buildPresetsCategory(ConfigBuilder builder, ConfigEntryBuilder eb, CustomDropsConfig config) {
        ConfigCategory presets = builder.getOrCreateCategory(Component.translatable("customdrops.config.category.presets"));
        ...
        presets.addEntry(eb.startTextDescription(Component.translatable("customdrops.config.presets.description")).build());

        presets.addEntry(eb.startSelector(
                Component.translatable("customdrops.config.presets.active"),
    ...
    private void applyPresetSelection(CustomDropsConfig config, String selected) {
        config.clearAll();
